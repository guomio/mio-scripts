...
const os = require('os');
const immer = require('@guomio/mio-scripts-utils/immer').produce;
const globby = require('@guomio/mio-scripts-utils/globby').sync;
...
function verifyTypeScriptSetup() {
  ...
  const compilerOptions = {
    ...
    moduleResolution: {
      ...
      value: 'node',
      reason: 'to match resolution',
    },
    resolveJsonModule: { value: true, reason: 'to match loader' },
    isolatedModules: { value: true, reason: 'implementation limitation' },
    paths: { value: undefined, reason: 'aliased imports are not supported' },
    baseUrl: { value: undefined, reason: 'aliased imports are not supported' },
  };
  ...
  if (!fs.existsSync(paths.appTypeDeclarations)) {
    fs.writeFileSync(
      ...
      `/// <reference types="@guomio/mio-cli" />${os.EOL}`,
    );
  }
}
