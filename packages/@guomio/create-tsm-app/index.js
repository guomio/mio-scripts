...
class BackManager {
  constructor(rootDir) {
    if (rootDir.endsWith('@guomio/create-tsm-app')) {
      logger.warn('can not override create-tsm-app itself');
      return;
    }
...
async function action(dir, opts) {
  ...
  try {
    ...
    await pkgtool.install(rootDir, ['@guomio/mio-tsm', 'pm2']);
    ...
    await executeNodeScript(
      ...
      [pkgtool.tool],
      `require('@guomio/mio-tsm/scripts/init.js').apply(null, JSON.parse(process.argv[1]));`,
    );
  } catch (err) {
    logger.error('@guomio/mio-tsm init error');
    console.log(err);
  }
}
...
commander
  ...
  .description(chalk.cyan('Create tsm starter'))
  .arguments('[directory]')
  .action(action)
  .parse(process.argv);
